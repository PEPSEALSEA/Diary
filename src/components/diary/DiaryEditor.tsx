...
export default function DiaryEditor({ user, onEntryChange, initialDate, refreshTrigger = 0 }: DiaryEditorProps) {
    ...
    // Refs to track state and avoid unnecessary re-renders/overwrites
    const lastSavedDiff = useRef({ title, content, privacy, date });
    const prevDate = useRef(date);
    const isInitialLoad = useRef(true);
    const explicitlySelectedEntry = useRef<string | null>(null);
    ...
    useEffect(() => {
        // 1. Handle Date Change: If the date changed, we ALWAYS want to load the entries for that new date
        if (prevDate.current !== date) {
            prevDate.current = date;
            explicitlySelectedEntry.current = null; // Reset explicit selection on date change
        ...
        // 2. Handle Initial Load: When data first arrives, pick the best entry
        if (isInitialLoad.current && entriesData) {
        ...
        // If data refreshed in background but we are already focused on an entry (or a new one),
        // we should NOT overwrite the form because the user might be typing.
        // We only update if we don't have an entryId yet and a new one might have appeared for this date.
        if (!entryId && !isDirty && entriesForDate.length > 0 && !explicitlySelectedEntry.current) {
            // Only auto-load if we are on an empty "New Entry" state and haven't typed anything
            loadIntoForm(entriesForDate[0]);
        }
    ...
    const handleEntrySelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        explicitlySelectedEntry.current = id || 'NEW_ENTRY'; // Track that user made a choice
        if (id) {
    ...
    const handleAutoSave = async () => {
        ...
        try {
            ...
            if (res && res.success) {
                ...
                lastSavedDiff.current = { title, content, privacy, date };
                setIsDirty(false);
                // onEntryChange(); // REMOVED: Don't trigger global refresh on auto-save to avoid text jumping
            }
        } catch (e) {
